...
TEST_CASE("PlaneEstimationTests.testEstimatePlane") {
    ...
    CHECK_NEAR(result.plane.position.x, 3.03, 0.1);
    CHECK_NEAR(result.plane.position.y, 1.0, 0.1);
    CHECK_NEAR(result.plane.position.z, 0.266, 0.1);
    
    // Dot the result with the expected value and compare the absolute value to 1 to ensure they're roughly identical.
    CHECK_NEAR(std::abs(standard_cyborg::toVector3f(result.plane.normal).transpose() * Eigen::Vector3f(0.0, 1.0, 0.0)), 1.0, 0.01);
    ...
    CHECK_NEAR(result.rmsProjectedDistance, 0.0f, 1e-6);
    
    CHECK(result.converged);
    
    CHECK(*result.planeVertices == standard_cyborg::sc3d::VertexSelection(7, {3, 4, 6}));
}

TEST_CASE("PlaneEstimationTests.testEstimatePlaneFromPointCloud") {
    std::vector<Vec3> positions;
    int numPts = 10000;
    ...
    CHECK_LT(projectedDistanceFromActualPositionToEstimatedPlane, 1e-3);
    ...
    CHECK_LT(Vec3::angleBetween(-result.plane.normal, normal), 1e-2);
}
